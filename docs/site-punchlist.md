...
- [x] **Section photo (Image #4) → condensed Berry Good proof card — DONE (2026-08-03, Charlie's call).** Rather than swap in another photo, replaced the AI-generated stock collaboration image (the "generated by AI" filler failure mode) with a condensed, interactive miniature of the `/consulting` Berry Good worked example: 5 facet tabs (Brand → Storefront → Journey → Operations → AI) over the real product-proof imagery, labeled "Example experience." Pairs the four abstract capabilities on the left with concrete proof on the right; existing "Explore how we help →" CTA leads to the full module. New `BerryGoodTeaser.tsx` reusing exported `berryViews`/`ViewVisual`. Verified desktop + mobile + all panels. (Old `studio-collaboration-wide.webp` still used on `/consulting` "Who you'll work with" — separate slot, left for later.)
