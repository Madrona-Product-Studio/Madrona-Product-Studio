...
- [x] **"Solve the system, not the symptom" — indexed (2026-08-16).** It was live + routed + linked from other articles but never added to `prerender.mjs`, so it had no static meta/OG and was absent from the sitemap. Added its prerender entry (12→13 routes) + OG card (`source` motif, `og-pov-solve-the-system.png`).
- [x] **OG cards Tier-2 polish (2026-08-16)** — descriptor line + enlarged motif across all 5 /thinking cards (direction B).
- [x] **Article resource links, Tier 1/2 (2026-08-16)** — under-the-hood + agentic "free to take"/"open source" → madrona-open repo & operating-system plugin; starter guide's 5 setup tools (GitHub, Claude Code, Codex, Vercel, claude.ai) → homepages. NOTE `/open` now has thinner on-site inbound linking (essay CTAs moved to GitHub, per "link GitHub until /open is redesigned"). Article prose lives in `MadronaV2*Note.tsx` components, not `thinking-content.json` (prerender SEO mirror only).
- [ ] **Article resource links, Tier 3 (next)** — product proof links (San Juan boating guide, Lila Trips, Garden HQ, Aria → /apps or live product; Helm only via `?demo=1`) + cross-article links (Thesis/starter/agentic name-drop each other unlinked).
