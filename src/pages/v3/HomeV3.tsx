...
// Proof cards: every live product is an outbound link to the real thing.
// Berry Good is not in the product portfolio (it is the demonstration
// business, not a studio product), so its card goes to the live storefront
// rather than /apps, where it does not exist.
const PROOF: [string, string, string, string, boolean][] = [
  [helmImage, "Helm", "Beta · Our command center", "/apps", false],
  [lilaImage, "Lila Trips", "Live · Adventure travel", "https://lilatrips.com", true],
  [sanJuanImage, "San Juan Boating Guide", "Live · Salish Sea routes", "https://www.sjiboating.com/", true],
  [berryImage, "Berry Good", "Demo · A connected farm operation", BERRY_URL, true],
];
...
export default function HomeV3() {
  ...
  return <main className="m2 v3">
    ...
    <Reveal as="section" className="v3-section v3-shell v3-proof"><div className="v3-help-head"><p className="v3-kicker">Proof in the work</p><h2>We build products, <span>and we run them.</span></h2><p className="v3-help-lede">Our own apps keep our product judgment close to real users, from live travel products to the demonstration farm.</p></div><div className="v3-proof-grid">{PROOF.map(([src,name,status,href,external]) => external ? <a className="v3-proof-card" key={name} href={href} target="_blank" rel="noreferrer" onClick={outboundClick(href, "home-proof")}><img src={src} alt={`${name} product interface`} /><figcaption><div><strong>{name}</strong><span>{status}</span></div><i aria-hidden="true">↗︎</i></figcaption></a> : <Link className="v3-proof-card" key={name} to={href}><img src={src} alt={`${name} product interface`} /><figcaption><div><strong>{name}</strong><span>{status}</span></div><i aria-hidden="true">→</i></figcaption></Link>)}</div><Link className="v3-practice-link v3-proof-all" to="/apps">See all products <span aria-hidden="true">→</span></Link></Reveal>
