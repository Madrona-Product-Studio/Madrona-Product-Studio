...
export default function ServicePageV3({ serviceId }: { serviceId: ServiceId }) {
  ...
  return (
    <main className="m2 v3">
      ...
      <section className="v3-start v3-shell"><div><p className="v3-kicker">How we start</p><h2>A small first move with visible payback.</h2></div><div><p>{service.startingPoint}</p><div className="v3-start-links"><a className="v3-btn v3-btn-primary" href={bookHref()} {...bookProps()} onClick={bookClick}>Book a free 30-minute call</a>{service.tryIt && <Link to={service.tryIt.to}>{service.tryIt.label} →</Link>}{service.demos && <Link to={service.demos.to}>{service.demos.label} →</Link>}{service.pov && <Link to={service.pov.to}>{service.pov.label} →</Link>}</div></div></section>
